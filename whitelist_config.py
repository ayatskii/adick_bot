"""
Whitelist Configuration for Telegram Bot
=========================================

This file contains the whitelist configuration that controls user access to the bot.
The bot can modify this file directly to make permanent changes to the whitelist.
"""

# Whitelist Configuration (ENABLED)
# Add user IDs of authorized users here
AUTHORIZED_USER_IDS = [
    546321644,
]

# Add usernames of authorized users here (without @ symbol)
AUTHORIZED_USERNAMES = [
]

# Optional: Enable/disable whitelist
ENABLE_WHITELIST = True  # Set to True to allow only authorized users

# Admin Configuration
ADMIN_USER_IDS = [
    546321644,
    1235660806,
]

# Admin Features
ENABLE_ADMIN_PANEL = True  # Set to False to disable admin features

